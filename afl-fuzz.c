...
#include <setjmp.h>
#include <stdarg.h>
#include "criu4snpsfuzzer/lib/c/criu.h"
#include "criu4snpsfuzzer/soccr/soccr.h"
...
struct queue_entry {
  ...
  region_score_t *regions_scores;     /* SNPSFuzzer: Score each region*/
...
/* SNPSFuzzer-specific variables */
khash_t(m32)* km32_state_snapshot;                        //hashtable to determine if the state has snapshot
klist_t(lms)* kl_messages_m1, *kl_messages_m2_m3;         //Divide the message into M1 and M23
u8 use_snapshot;                                          //use snapshot flag
u8 has_snapshot_flag = 0;                                 //has snapshot flag
u8 can_snapshot_flag = 0;                                 //can take snapshot flag 
u8 is_restoring = 0;                                      // whether is restoring
u32 restore_tms;                                          // restore timeout (ms)
u8 is_first_run_of_fuzz_one = 0;

s32 criu_dump_monitor_pid;                                //pid of process snapshot taken

/*log file*/
extern FILE* snapshot_logfile;                            //log_file
s32 snapshot_logfile_fd;                                  //log_file_fd
/*Prefix messgaes count file*/
FILE* prefix_save_logfile;
s32 prefix_save_logfile_fd;

u32 messages_sent_m1 = 0;                                 //the number of Message M1
u32 messages_sent_m23 = 0;                                //the number of Message M23
u32 messages_sent_total = 0;
u32 messages_sent_total_last = 0;
int socket_fd = -1;                                       //TCP/UDP Scoket id
union libsoccr_addr src;
socklen_t src_len;
union libsoccr_addr dst;
socklen_t dst_len;
u32 seq,ack;
struct tcp_info ti;
u32 mss_clamp;
u32 src_wscale, dst_wscale;
u32 timestamp;
socklen_t onr;
...
char* queue_r;
char* queue_s;
char* iptables_add,*iptables_del;
int child_pid_snapshot;
u32 run_target_count = 0;
u32 run_target_count_current;
u32 total_execs_current;
u32 try_restore_count = 0;
u32 try_restore_count_current;
u32 restore_ok_count = 0;
u32 restore_ok_count_current;
u8 backstep;
u8 backstep_count;
u8 serious_err_count;
...
/*M1 M23 response_buf response_buf_size*/
char* response_buf_m1 = NULL;
int response_buf_size_m1 = 0;                             //the size of the whole M1 response buffer
u32* response_bytes_m1 = NULL;                            //an array keeping accumulated response buffer size for M1

char* response_buf_m23 = NULL;
int response_buf_size_m23 = 0;                            //the size of the whole M23 response buffer
u32* response_bytes_m23 = NULL;                           //an array keeping accumulated response buffer size for M23

typedef enum SNPSF_RES{
  OK, 
  M1_TIMEOUT         = 0x1, 
  M1_NOPUT           = 0x2,
  DUMP_AFL_FAILED    = 0x4, 
  DUMP_FAILED_ERR    = 0x8,
  RESTORE_AFL_FAILED = 0x10,
  RESTORE_TIMEOUT    = 0x20,
  RESTORE_FAILED     = 0x40,
  M23_TIMEOUT        = 0x80
} sres;

struct timeval log_time;
time_t log_time_start = 0;
char snapshot_log_t[1024];
int snapshot_log_t_len;
int snapshot_log_level=0;
int snapshot_log_level_idx;


#define SNAPSHOT_LOG_ENTER(x...)do{\
for(snapshot_log_level_idx=0;snapshot_log_level_idx<snapshot_log_level;snapshot_log_level_idx++) write(snapshot_logfile_fd,"| ",2);\
gettimeofday(&log_time,NULL);\
snapshot_log_t_len = sprintf(snapshot_log_t,"@enter %s---%ld\n",x,log_time.tv_sec*1000+log_time.tv_usec/1000-log_time_start);\
write(snapshot_logfile_fd,snapshot_log_t,snapshot_log_t_len);\
snapshot_log_level++;\
}while(0)

#define SNAPSHOT_LOG_LEAVE(x...)do{\
snapshot_log_level--;\
for(snapshot_log_level_idx=0;snapshot_log_level_idx<snapshot_log_level;snapshot_log_level_idx++) write(snapshot_logfile_fd,"| ",2);\
gettimeofday(&log_time,NULL);\
snapshot_log_t_len = sprintf(snapshot_log_t,"@leave %s---%ld\n",x,log_time.tv_sec*1000+log_time.tv_usec/1000-log_time_start);\
write(snapshot_logfile_fd,snapshot_log_t,snapshot_log_t_len);\
}while(0)

#define SNAPSHOT_LOG(x...) do{\
for(snapshot_log_level_idx=0;snapshot_log_level_idx<snapshot_log_level;snapshot_log_level_idx++) write(snapshot_logfile_fd,"| ",2);\
SNAPSHOT_LOG_UNLEVELED(x);\
}while(0)

#define SNAPSHOT_LOG_UNLEVELED(x...) do{\
snapshot_log_t_len = sprintf(snapshot_log_t,x);\
write(snapshot_logfile_fd,snapshot_log_t,snapshot_log_t_len);\
}while(0)
...
/*limit M23 size*/
u32 MAX_BUF_SIZE = 0;                                     //MAX_BUF_SIZE=0: no limit M23 size
// u32 MAX_BUF_SIZE = 99999999;
s32 MAX_MESSAGES_COUNTS = 5;                              //if MAX_BUF_SIZE=99999999: limit M23 size within 5 messages size
u32 now_buf_size = 0;                                     //used to limit MAX_BUF_SIZE

/*MCA algorithm*/
s32 current_choose_position = -1;                         //MCA Algorithm current choose position
u32 SCORE_BETA = 1000;                                    //MCA Algorithm position score parameter
u8 mca_algo = MCA_FIRST;

u32 send_over_network_m1_length;                          //send_over_network the number of M1

/*energy assign*/
u32 stage_max_factor = 1;                                 //control energy assign parameter
u32 MAX_STAGE_CUR = 16;                                   //control energy assign 

/*bitmap*/
u8 trace_bits_m1[MAP_SIZE];                               //Record the bitmap after sending M1

/*tcp-based*/
uint32_t fix_seq_num, fix_ack_num;

/*timer*/
struct timeval timeA, timeB;
long int time_record[4]; // 0:time_send_m1, 1:time_send_m23, 2:time_restore, 3:time_dump
const char* time_record_table[4] = {"time_send_m1","time_send_m23","time_restore","time_dump"};
int minchange = 5;
/*End of SNPSFuzzer*/

s32 check_state_snapshot()
{
  // int ret;
  //don't have snapshot
  if (kh_get(m32, km32_state_snapshot, target_state_id) == kh_end(km32_state_snapshot)) {
    // kh_put(m32,km32_state_snapshot,state_id,&ret);
    return 0;
  } else return 1;
...
void setup_ipsm()
{
  ...
  if (use_snapshot) km32_state_snapshot = kh_init(m32);  //initialization
}
...
void destroy_ipsm()
{
  ...
  if (use_snapshot) kh_destroy(m32, km32_state_snapshot);
...
//MCA algorithm
s32 message_chain_analysis(s32 *M2_start_region_ID_PREV, s32 M2_start_region_ID_PREV_index, u8 mca_algo)
{
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG_ENTER("message_chain_analysis()");
  #endif
  /* mca_algorithm1: choose the first target region, Initial selection algorithm for aflnet*/
  if (mca_algo == MCA_FIRST) {
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("MCA_FIRST: choose FIRST position\n");
    #endif
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG_LEAVE("message_chain_analysis()");
    #endif
    return  M2_start_region_ID_PREV[0];
  }
  /* mca_algorithm2: choose the last target region*/
  else if (mca_algo == MCA_LAST) {
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("MCA_LAST: choose LAST position\n");
    #endif
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG_LEAVE("message_chain_analysis()");
    #endif
    return  M2_start_region_ID_PREV[M2_start_region_ID_PREV_index - 1];
  }

  /* mca_algorithm3: smart choose the target region*/
  else if (mca_algo == MCA_SMART) {
    /* Case 1 : Only one region*/
    /* Case 2 : Do choose_last_position for a few cycles to get enough statistical information*/
    if (M2_start_region_ID_PREV_index == 1 || queue_cycle < 6) {
      // if(M2_start_region_ID_PREV_index == 1) {
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("MCA_SMART: choose first position score:%d\n", queue_cur->regions_scores[M2_start_region_ID_PREV[0]].score);
      #endif
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG_LEAVE("message_chain_analysis()");
      #endif
    ...
    u32 *region_scores = NULL;
    region_scores = (u32 *)ck_alloc(M2_start_region_ID_PREV_index * sizeof(u32));
    if (!region_scores) PFATAL("Cannot allocate memory for state_scores");

    s32 result = 0, i;
    for (i = 0; i < M2_start_region_ID_PREV_index; i++) {

      queue_cur->regions_scores[M2_start_region_ID_PREV[i]].score = ceil(SCORE_BETA * (queue_cur->regions_scores[M2_start_region_ID_PREV[i]].message_depth + 1) 
        * log(queue_cur->regions_scores[M2_start_region_ID_PREV[i]].paths_discovered + 2) / log(queue_cur->regions_scores[M2_start_region_ID_PREV[i]].fuzzs + 2));

      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("fuzzs:%d--", queue_cur->regions_scores[M2_start_region_ID_PREV[i]].fuzzs);
      SNAPSHOT_LOG("selected_times:%d--", queue_cur->regions_scores[M2_start_region_ID_PREV[i]].selected_times);
      SNAPSHOT_LOG("paths_discovered:%d--", queue_cur->regions_scores[M2_start_region_ID_PREV[i]].paths_discovered);
      SNAPSHOT_LOG("message_depth:%d--", queue_cur->regions_scores[M2_start_region_ID_PREV[i]].message_depth);
      SNAPSHOT_LOG("score:%d\n", queue_cur->regions_scores[M2_start_region_ID_PREV[i]].score);
      #endif

      if (i == 0) {
        region_scores[i] = queue_cur->regions_scores[M2_start_region_ID_PREV[i]].score;
      } else {
        region_scores[i] = region_scores[i - 1] + queue_cur->regions_scores[M2_start_region_ID_PREV[i]].score;
      }
    }
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("\n");
    #endif
    u32 randV = UR(region_scores[M2_start_region_ID_PREV_index - 1]);
    u32 idx = index_search(region_scores, M2_start_region_ID_PREV_index, randV);
    result = M2_start_region_ID_PREV[idx];

    if (region_scores) ck_free(region_scores);
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG_LEAVE("message_chain_analysis()");
    #endif
    return result;
  }
  else PFATAL("error mca_algo value");
}
...
void update_state_aware_variables(struct queue_entry *q, u8 dry_run)
{
  ...
  if (!dry_run) {
    ...
    //update value paths_discovered of the corresponding region
    if (state_aware_mode && use_snapshot && (mca_algo == MCA_SMART) && (current_choose_position != -1)) {
      queue_cur->regions_scores[current_choose_position].paths_discovered++;
    }
  }
...
void timer_start(struct itimerval *it, u32 tms) {
  it->it_value.tv_sec = (tms / 1000);
  it->it_value.tv_usec = (tms % 1000) * 1000;
  setitimer(ITIMER_REAL, it, NULL);
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("set timer for %u ms\n",tms);
  #endif
}

void timer_close(struct itimerval *it) {
  it->it_value.tv_sec = 0;
  it->it_value.tv_usec = 0;
  setitimer(ITIMER_REAL, it, NULL);
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("clear timer\n");
  #endif
}

// 0:time_send_m1, 1:time_send_m23, 2:time_restore, 3:time_dump
void clock_start() {
  gettimeofday(&timeA, NULL);
}

// 0:time_send_m1, 1:time_send_m23, 2:time_restore, 3:time_dump
void clock_close(int time_select) {
  gettimeofday(&timeB, NULL);
  time_record[time_select] = timeB.tv_sec * 1000 + timeB.tv_usec / 1000 - timeA.tv_sec * 1000 - timeA.tv_usec / 1000;
  fprintf(prefix_save_logfile,"%s:%ld\n", time_record_table[time_select], time_record[time_select]);
  fflush(prefix_save_logfile);
}

void clock_close_depends(int time_select, int res) {
  if(!res){
    gettimeofday(&timeB, NULL);
    time_record[time_select] = timeB.tv_sec * 1000 + timeB.tv_usec / 1000 - timeA.tv_sec * 1000 - timeA.tv_usec / 1000;
    fprintf(prefix_save_logfile,"%s:%ld\n", time_record_table[time_select], time_record[time_select]);
...
void get_popen_result(const char *mes, int **res_code, int *size)
{ 
  //get popen results
  ...
  FILE *fp;
  if ((fp = popen(mes, "r")) == NULL) {
    PFATAL("error execute ps -ef");
  }
  int fp_fd = fileno(fp);
  ...
  SNAPSHOT_LOG("%s\n",mes);
  #endif
  int n = read(fp_fd,buf,4096);
  pclose(fp);

  int ret = 0;
  if(n>0){
    buf[n-1]='\0'; // remove the last \n, avoiding memory overflow
    char* token;
    char* tmp = buf;
    while ((token = strsep(&tmp, "\n")) != NULL) {
      ...
      ret ++;
      *res_code = (int *) ck_realloc(*res_code, ret * sizeof(int));
      (*res_code)[ret-1] = atoi(token);
    }
  }else{
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("  none found\n");
    #endif
  }
  *size = ret;
}

int get_popen_result_no_return(const char *mes)
{ 
  //get popen results
  char buf[4096];
...
int check_target_process(){
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("now check all target process...\n");
  #endif
  char* ttt = alloc_printf("ps -ef | grep %s| grep -v grep| grep -v afl-fuzz | grep -E '%d|%d'",use_banner, getpid(), forksrv_pid);

  int ret = get_popen_result_no_return(ttt);
  ck_free(ttt);
  return ret;
}
...
int check_zombies() { 
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("now check all zombies...\n");
  #endif
  char* ttt = alloc_printf("ps -ef | grep \"<defunct>\" | grep -v grep | grep -E '%d|%d'", getpid(), forksrv_pid);
  int ret = get_popen_result_no_return(ttt);
  ck_free(ttt);
  return ret;
}
...
void clear_zombies() {
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("call clear_zombies...\n");
  #endif
  if(check_zombies()){                  //If there is a zombie process
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("exist, try waitpid\n");
    #endif
    /* get zombie Pid */
    char* _x= alloc_printf("ps -ef | grep \"<defunct>\" | grep -v grep | grep -E '%d|%d' | awk '{ print $2 }'", getpid(), forksrv_pid);
    FILE *fp;
    if ((fp = popen(_x, "r")) == NULL) {
      PFATAL("error execute ps -ef");
    }
    ck_free(_x);
    char buf[1024];                     // It must be ensured that the total length of the result of 'ps' is not greater than 1024 (high probability)
    int buf_len = read(fileno(fp),buf,1024);
    pclose(fp);

    /* use waitpid for all zombie processes */
    if(buf_len>0){
      buf[buf_len-1]='\0';
      char* dunfunct_pid;
      char* p_buf = buf;
      while((dunfunct_pid=strsep(&p_buf,"\n"))!=NULL){
        #ifdef SNAPSHOT_DEBUG
        SNAPSHOT_LOG("zombie pid:%s\n", dunfunct_pid);
        #endif
        int criu_defunct_pid = atoi(dunfunct_pid);
        if (waitpid(criu_defunct_pid, NULL, 0) < 0) {
          PFATAL("waitpid error!");
        }
      }
    }

    /* If there are still zombies, there are problems with the code or program */
    if(check_zombies()){
      PFATAL("cannot clear zombies! check your code!");
    }
  }else{
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("none zombie\n");
    #endif
  }
...
void clear_child() {
  #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("call clear_child...\n");
  #endif
  if(!kill(child_pid,0)){            // if the child process is still alive
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("exist, try kill(%d,SIGKILL)\n",child_pid);
    #endif
    kill(child_pid,SIGKILL);
  }else{
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("none child\n");
    #endif
  }
}
...
void check_process() {
  ...
  SNAPSHOT_LOG("now check all target process...\n");
  #endif
  ttt = alloc_printf("ps -ef | grep %s| grep -v grep| grep -v afl-fuzz",use_banner);
  get_popen_result_no_return(ttt);
  ck_free(ttt);
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("now check all zombies...\n");
  #endif
  ttt = alloc_printf("ps -ef | grep \"<defunct>\" | grep -v grep");
  get_popen_result_no_return(ttt);
  ck_free(ttt);
}
...
void what_err_ret_mean(int ret)
{
  ...
  case -EBADE:
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("RPC has returned fail\n");
    #endif
    break;
  case -ECONNREFUSED:
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("Unable to connect to CRIU\n");
    #endif
    break;
  case -ECOMM:
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("Unable to send/recv msg to/from CRIU\n");
    #endif
    break;
  case -EINVAL:
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("CRIU doesn't support this type of request."
      "You should probably update CRIU\n");
    #endif
    break;
  case -EBADMSG:
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("Unexpected response from CRIU."
      "You should probably update CRIU\n");
    #endif
    break;
  default:
  {
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("Unknown error type code. You should probably update CRIU\n");
    #endif
  }
  }
}
...
// throw errors
int dump_afl_socket() {
  if(net_protocol == PRO_UDP) return OK;
  sres res = OK;
  // libsoccr requires that no packets flow in during the entire P/R process, so block iptables before suspending afl-socket
  // Accordingly, remember to unblock when exiting
  system(iptables_add);
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("forbid iptables\n");
  #endif

  // Pause AFL sockets
  so = libsoccr_pause(socket_fd);
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("pause afl-socket OK\n");
  #endif

  // Store other data of AFLsocket
  dsize = libsoccr_save(so, &data, sizeof(data));
  if (dsize < 0) {
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("libsoccr_save(so, &data, sizeof(data)) FAILED\n");
    #endif
    res = DUMP_AFL_FAILED;
  }
  close(socket_fd); 
  ...
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("save afl-socket OK\n");
  #endif
  system(iptables_del);
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("free iptables\n");
  #endif

  return res;
}

int restore_afl_socket() {
  if(net_protocol == PRO_UDP) return OK;
  ...
  // Set the local and peer addresses of the socket
  ...
  try{
    int res_restore = 0;
    if ((res_restore = libsoccr_restore(so_rst, &data, dsize))) {
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("libsoccr_restore failed\n");
      #endif
      goto err_out;
    }
  }catch{
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("libsoccr_restore big failed\n");
    #endif
    goto err_out;
  }
  ...
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("restore afl-socket ok\n");
  #endif
...
err_out:
  ...
  SNAPSHOT_LOG("free iptables\n");
  #endif
  return RESTORE_AFL_FAILED;
}

s32 self_criu_restore()
{
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG_ENTER("self_criu_restore()");
  #endif
  try_restore_count++;
  try_restore_count_current++;
  s32 fork_pid;

  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("Step I: try restore afl-socket\n");
  #endif

  sres __res = restore_afl_socket();

  if(__res>0){
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG_LEAVE("self_criu_restore(restore_afl_socket() Fault)");
    #endif
    //PFATAL("libsoccr failed");
    return __res;
  }
  
  /* For Proftpd only */
  if (strcmp(use_banner,"proftpd") == 0) {
    int fd = open("proftpd.scoreboard.lck", O_RDONLY | O_CREAT, 0644);
    if (fd < 0) PFATAL("Unable to create proftpd.scoreboard.lck");
    close(fd);
    fd = open("proftpd.scoreboard", O_RDONLY | O_CREAT, 0644);
    if (fd < 0) PFATAL("Unable to create proftpd.scoreboard");
    close(fd);
  }

  /* Open image file */
  int img_fd;
  u8 *temp = alloc_printf("snapshot_dir/state%d_images", target_state_id);
  img_fd = open(temp, O_DIRECTORY);
  if (img_fd < 0) {
    PFATAL("Can't open %s", temp);
  }
  ck_free(temp);

    // Set the RESOTRE parameter
    criu_init_opts();
    criu_set_shell_job(true);
    criu_set_log_level(4);
    criu_set_log_file("rst.log");
    if (net_protocol == PRO_TCP) criu_set_tcp_established(true);
    criu_set_images_dir_fd(img_fd);
    criu_set_ext_unix_sk(true); 

    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("Step II: try criu_restore_child(), target state: %d\n",target_state_id);
    #endif
    is_restoring = 1;
    fork_pid = criu_restore_child();
    close(img_fd);

    // Timeout failure
    if(is_restoring==2){
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("Timeout Fault\n");
      #endif
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("try kill -9 %d\n",child_pid);
      #endif
      clear_child();
      clear_zombies();
      if(!kill(child_pid,0)){
      ...
      #endif
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG_LEAVE("self_criu_restore(Timeout Fault)");
      #endif
    ...
    if (fork_pid <= 0) {
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("Restore Failed Fault\n");
      #endif
      what_err_ret_mean(fork_pid);
      clear_child();
      clear_zombies();
      if(!kill(child_pid,0)){
        PFATAL("undead child!");
      ...
      #endif
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG_LEAVE("self_criu_restore(Restore Failed Fault)");
    #endif
    return RESTORE_FAILED;
}

  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("restore child ok\n");
  #endif

  restore_ok_count++;
  restore_ok_count_current++;
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG_LEAVE("self_criu_restore()");
  #endif
  return OK;
}

int send_over_network_m1()
{
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG_ENTER("send_over_network_m1()");
  #endif
  ...
  if (response_buf_m1) {
    ck_free(response_buf_m1);
    response_buf_m1 = NULL;
    response_buf_size_m1 = 0;
  }

  if (response_bytes_m1) {
    ck_free(response_bytes_m1);
    response_bytes_m1 = NULL;
  }

  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("Finish clear the response buffer and reset the response buffer size\n");
  #endif
  ...
  if (connect(sockfd, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) {
    ...
    if (n == 1000) {
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("handshake failed 1000 times\n");
      #endif
      goto HANDLE_RESPONSES_M1;
    }
  }

  // Backup the local and peer IP addresses in the socket of afl
  n = 0;
  do{
    if (getsockname(socket_fd, (struct sockaddr *)&src, &src_len)) {
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("getsockname(socket_fd, (struct sockaddr *)&local, &dst_len) FAILED\n");
      #endif
    }
    if (getpeername(socket_fd, (struct sockaddr *)&dst, &dst_len)) {
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("getpeername(socket_fd, (struct sockaddr *)&dst, &dst_len) FAILED\n");
      #endif
    }
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("src=%p, ip = %s, port = %u \n",&src, inet_ntoa(p_src->sin_addr), ntohs(p_src->sin_port));
    SNAPSHOT_LOG("dst=%p, ip = %s, port = %u \n",&dst, inet_ntoa(p_dst->sin_addr), ntohs(p_dst->sin_port));
    #endif
    n++;
    if(n==1000){
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("get socket addresses failed\n");
      #endif
      goto HANDLE_RESPONSES_M1;
    }
  }while(p_src->sin_port==0);

  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("handshake ok\n");
  #endif

  //retrieve early server response if needed
  if (net_recv(sockfd, timeout, poll_wait_msecs, &response_buf_m1, &response_buf_size_m1)) PFATAL("send_over_network_m1 retrieve early server response error");

  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("retrieve early server response finished\n");
  #endif

  //write the request messages
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("start to write request messages and recv response\n");
  #endif
  kliter_t(lms) *it;
  messages_sent_m1 = 0;
  for (it = kl_begin(kl_messages_m1); it != kl_end(kl_messages_m1); it = kl_next(it)) {
    n = net_send(sockfd, timeout, kl_val(it)->mdata, kl_val(it)->msize);

    messages_sent_m1++;
    //Allocate memory to store new accumulated response buffer size
    response_bytes_m1 = (u32 *)ck_realloc(response_bytes_m1, messages_sent_m1 * sizeof(u32));

    //Jump out if something wrong leading to incomplete message sent
    if (n != kl_val(it)->msize) {
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("error:send_over_network_m1 send error\n");
      #endif
      goto HANDLE_RESPONSES_M1;
    }

    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("send message %d ok\n",messages_sent_m1);
    #endif

    //retrieve server response
    // u32 prev_buf_size = response_buf_size_m1;
    if (net_recv(sockfd, timeout, poll_wait_msecs, &response_buf_m1, &response_buf_size_m1)) {
      // PFATAL("send_over_network_m1 recv error");
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG( "send_over_network_m1 recv error\n");
      #endif
      goto HANDLE_RESPONSES_M1;
    }
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("recv response %d ok\n",messages_sent_m1);
    #endif

    //Update accumulated response buffer size
    response_bytes_m1[messages_sent_m1 - 1] = response_buf_size_m1;
  ...
  if(response_buf_size_m1==0){
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("No Response Fault\n");
    #endif
    goto HANDLE_RESPONSES_M1;
  }
  ...
  // The process under test crashes
  if(kill(child_pid,0)){
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("Child Not Exist Fault!\n");
    #endif
    __res = M1_NOPUT;
    goto HANDLE_RESPONSES_M1;
  }
  ...
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG_LEAVE("send_over_network_m1");
  #endif

  messages_sent = messages_sent_m1;
  if(response_buf_size_m1>0){
    response_buf = (char *)ck_realloc(response_buf, (response_buf_size_m1 + 1) * sizeof(char));
    memcpy(response_buf, response_buf_m1, response_buf_size_m1 + 1);
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("copy response_buf_m1 to respense_buf OK\n");
    #endif

    response_buf_size = response_buf_size_m1;
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("response_buf_size = response_buf_size_m1;\n");
    #endif
  }
  
  response_bytes = (u32 *)ck_realloc(response_bytes, messages_sent_m1 * sizeof(u32));
  return OK;
HANDLE_RESPONSES_M1:

  net_recv(sockfd, timeout, poll_wait_msecs, &response_buf_m1, &response_buf_size_m1);

  if (messages_sent_m1 > 0 && response_bytes_m1 != NULL) {
    response_bytes_m1[messages_sent_m1 - 1] = response_buf_size_m1;
  }
  ...
  if (terminate_child && (child_pid > 0)) {
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("terminate_child && (child_pid > 0), so I'll kill(%d, SIGTERM);\n",child_pid);
    #endif
  ...
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("start to kill(child_pid, 0) in loop...\n");
  #endif
  while (1) {
    int status = kill(child_pid, 0);
    if ((status != 0) && (errno == ESRCH)){
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("(status != 0) && (errno == ESRCH), break\n");
      #endif
      break;
    } 
  }
  ...
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("res = read(fsrv_st_fd, &status, 4) OK");
  #endif

  close(sockfd);
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("close(sockfd);\n");
  #endif

  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG_LEAVE("send_over_network_m1(HANDLE_RESPONSES_M1)");
  #endif
  return __res;
}

int send_over_network_m23()
{
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG_ENTER("send_over_network_m23");
  #endif
  int n;
  u8 likely_buggy = 0;
  //write the request messages
  kliter_t(lms) *it;
  messages_sent_m23 = messages_sent_m1;

  int sockfd = socket_fd;
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("copy sockfd: int sockfd = socket_fd;\n");
  #endif

  //add prefix messages send 

  // fprintf(prefix_save_logfile, "cur_time:%llu,  prefix_save:%d\n", get_cur_time() / 1000, messages_sent_m23);
  fprintf(prefix_save_logfile, "prefix_save:%d\n", messages_sent_m23);
  fflush(prefix_save_logfile);
  ...
  //Clear the response buffer and reset the response buffer size
  if (response_buf_m23) {
    ck_free(response_buf_m23);
    response_buf_m23 = NULL;
    response_buf_size_m23 = 0;
  }

  if (response_bytes_m23) {
    ck_free(response_bytes_m23);
    response_bytes_m23 = NULL;
  }

  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("clear response buffer and reset the response buffer size and response buffer and reset the response buffer size OK\n");
  #endif

  response_buf_m23 = (char *)ck_realloc(response_buf_m23, (response_buf_size_m1 + 1) * sizeof(char));
  memcpy(response_buf_m23, response_buf_m1, response_buf_size_m1 + 1);
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("copy response_buf_m1 to respense_buf_m23 OK\n");
  #endif

  response_buf_size_m23 = response_buf_size_m1;
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("response_buf_size_m23 = response_buf_size_m1;\n");
  #endif

  response_bytes_m23 = (u32 *)ck_realloc(response_bytes_m23, messages_sent_m1 * sizeof(u32));

  int _x;
  for (_x = 0; _x < messages_sent_m1; _x++) {
    response_bytes_m23[_x] = response_bytes_m1[_x];
  }
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("copy response_bytes_m1 to respense_bytes_m23 OK\n");
  SNAPSHOT_LOG("start to write request messages and recv response\n");
  #endif
  int m23_count = 0;
  for (it = kl_begin(kl_messages_m2_m3); it != kl_end(kl_messages_m2_m3); it = kl_next(it)) {
    m23_count++;
    n = net_send(sockfd, timeout, kl_val(it)->mdata, kl_val(it)->msize);
    // #ifdef SNAPSHOT_DEBUG
    // SNAPSHOT_LOG("send message mdata:%s, msize:%d \n",kl_val(it)->mdata, kl_val(it)->msize);
    // #endif
    messages_sent_m23++;

    //Allocate memory to store new accumulated response buffer size
    response_bytes_m23 = (u32 *)ck_realloc(response_bytes_m23, messages_sent_m23 * sizeof(u32));

    //Jump out if something wrong leading to incomplete message sent
    if (n != kl_val(it)->msize) {
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("send message %d error\n",messages_sent_m23);
      #endif
    ...
    //retrieve server response
    u32 prev_buf_size = response_buf_size_m23;
    if (net_recv(sockfd, timeout, poll_wait_msecs, &response_buf_m23, &response_buf_size_m23)) {
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("recv response %d error\n",messages_sent_m23);
      #endif
      goto HANDLE_RESPONSES;
    }
    #ifdef SNAPSHOT_DEBUG
    ...
    //Update accumulated response buffer size
    response_bytes_m23[messages_sent_m23 - 1] = response_buf_size_m23;

    //set likely_buggy flag if AFLNet does not receive any feedback from the server
    //it could be a signal of a potentiall server crash, like the case of CVE-2019-7314
    if (prev_buf_size == response_buf_size_m23) likely_buggy = 1;
    else likely_buggy = 0;
  }

  if(child_timed_out){
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("send_m23 timeout!\n");
    #endif
  }

  if (m23_count >= MAX_MESSAGES_COUNTS) {
    if (MAX_BUF_SIZE > now_buf_size) MAX_BUF_SIZE = now_buf_size;
  }


HANDLE_RESPONSES:
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("now HANDLE_RESPONSES\n");
  #endif

  net_recv(sockfd, timeout, poll_wait_msecs, &response_buf_m23, &response_buf_size_m23);
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("net_recv(sockfd, timeout, poll_wait_msecs, &response_buf_m23, &response_buf_size_m23); OK\n");
  #endif

  if (messages_sent_m23 > 0 && response_bytes_m23 != NULL) {
    response_bytes_m23[messages_sent_m23 - 1] = response_buf_size_m23;
  }

  response_buf = (char *)ck_realloc(response_buf, (response_buf_size_m23 + 1) * sizeof(char));
  memcpy(response_buf, response_buf_m23, response_buf_size_m23 + 1);
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("copy response_buf_m23 to response_buf\n");
  #endif
  response_buf_size = response_buf_size_m23;
  
  response_bytes = (u32 *)ck_realloc(response_bytes, messages_sent_m23 * sizeof(u32));
  // memcpy(response_bytes, response_bytes_m23, messages_sent_m23);
  int i;
  for (i = 0; i < messages_sent_m23; i++) {
    response_bytes[i] = response_bytes_m23[i];
  }
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("copy response_bytes_m23 to response_bytes\n");
  #endif

  messages_sent = messages_sent_m23;
  ...
  if (likely_buggy && false_negative_reduction) {
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("likely_buggy && false_negative_reduction, so I'll waitpid(child_pid, &status, 0)\n");
    #endif
    ...
    pid_t retpid = waitpid(child_pid, &status, 0);
    if (retpid <= 0 && errno != ECHILD)  PFATAL("waitpid() failed");
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("waitpid(%d, &status, 0); OK\n",child_pid);
    #endif
  }
  #ifdef SNAPSHOT_DEBUG
  check_process();
  ...
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG_LEAVE("send_over_network_m23()");
  #endif
  return status;
}
...
{
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG_ENTER("send_over_network");
  #endif
  int n;
  u8 likely_buggy = 0;
  struct sockaddr_in serv_addr;
  ...
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("clear the response buffer and reset the response buffer size\n");
  #endif
  ...
  if (net_recv(sockfd, timeout, poll_wait_msecs, &response_buf, &response_buf_size)) {
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("retrieve early server response error\n");
    #endif
    goto HANDLE_RESPONSES;
  }
  ...
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("start to write the request messages\n");
  #endif
...
HANDLE_RESPONSES:
  ...
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("net_recv(sockfd, timeout, poll_wait_msecs, &response_buf, &response_buf_size); OK\n");
  #endif
  ...
  //give the server a bit more time to gracefully terminate
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("kill(child_pid, 0) in loop...\n");
  #endif
  while(1) {
    int status = kill(child_pid, 0);
    if ((status != 0) && (errno == ESRCH)) {
  ...
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG_LEAVE("send_over_network()");
  #endif
  return 0;
}
...
static void add_to_queue(u8* fname, u32 len, u8 passed_det) {
  ...
  //initialize regions_scores 
  q->regions_scores = NULL;
  ...
  if (corpus_read_or_sync) {
    ...
    //initialize regions_scores 
    if (state_aware_mode && use_snapshot && (mca_algo == MCA_SMART)) {
      int i;
      // region_score_t *region_score_ptr = NULL;
      q->regions_scores = (region_score_t *)ck_alloc(q->region_count * sizeof(region_score_t));
      for (i = 0; i < q->region_count; i++) {
        // q->regions_scores = (region_score_t *)ck_realloc(q->regions_scores, (i+1) * sizeof(region_score_t));
        q->regions_scores[i].score = 1;
        q->regions_scores[i].message_depth = 0;
        q->regions_scores[i].paths_discovered = 0;
        q->regions_scores[i].fuzzs = 0;
        q->regions_scores[i].selected_times = 0;
      }
      // q->regions_scores = region_score_ptr;
    }
...
EXP_ST void destroy_queue(void) {
  ...
  while (q) {
    ...
    if (q->regions_scores) ck_free(q->regions_scores);
    ck_free(q);
    q = n;
...
// show every state infomation
void show_states_info(void)
{
  khint_t k;
  state_info_t *state;
  int i;
  for (i = 0; i < state_ids_count; i++) {
    ...
    if (k != kh_end(khms_states)) {
      state = kh_val(khms_states, k);
      ACTF("Now the state is %d", state_id);
      ACTF("\t is_covered:%d", state->is_covered);
      ACTF("\t paths:%d", state->paths);
      ACTF("\t paths_discovered:%d", state->paths_discovered);
      ACTF("\t selected_times:%d", state->selected_times);
      ACTF("\t fuzzs:%d", state->fuzzs);
      ACTF("\t score:%d", state->score);
      ACTF("\t selected_seed_index:%d", state->selected_seed_index);
      ACTF("\t seeds_count:%d", state->seeds_count);
    }
  }
}
...
EXP_ST void setup_shm(void) {
  ...
  OKF("will use %s as share memory...",g_shm_file_path);
  // tmpnam(g_shm_file_path);

  /* create the shared memory segment as if it was a file */
  g_shm_fd = shm_open(g_shm_file_path, O_CREAT | O_RDWR | O_EXCL, 0777);
  if (g_shm_fd == -1) {
...
EXP_ST void init_forkserver(char** argv) {
  ...
  if (!forksrv_pid) {
    ...
    if (use_snapshot) {
      if(!strcmp(use_banner,"openssl")){
        #ifdef SNAPSHOT_DEBUG
        SNAPSHOT_LOG("USE SOCKET HOOK!\n");
        #endif
        char *val;
        if ((val = getenv("LD_PRELOAD")) != NULL) {
          char *env_tmp = alloc_printf("%s:/usr/local/lib/hook_socket.so", val);
          setenv("LD_PRELOAD", env_tmp, 1);
          ck_free(env_tmp);
        }
        else {
          setenv("LD_PRELOAD", "/usr/local/lib/hook_socket.so", 0);
        }
      }
    }
...
{
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG_ENTER("self_criu_dump()");
  #endif

  // libsoccr failed
  if(dump_afl_socket()>0){ 
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("Libsoccr Fault\n");
    #endif
    clear_child();
    if(net_protocol==PRO_TCP)
      close(socket_fd);
    ...
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG_LEAVE("self_criu_dump(Libsoccr Fault)");
    #endif
    return DUMP_AFL_FAILED;
  }

  /* reset files */
  DIR *dirptr = NULL;
  u8 *dir_name = alloc_printf("snapshot_dir/state%d_images", target_state_id);
  dirptr = opendir(dir_name);
  if (dirptr == NULL) {
    if (mkdir(dir_name, 0755) == -1) PFATAL("mkdir %s error", dir_name);
  } else {
    closedir(dirptr);
  }
  u8 *rm_dir_name = alloc_printf("cd %s && rm -rf ./*", dir_name);
  system(rm_dir_name);
  ck_free(rm_dir_name);

  int return_code;
  if (strcmp(use_banner, "forked-daapd")!=0) {
    /* open img for dump */
    int img_fd = open(dir_name, O_DIRECTORY);
    if (img_fd < 0) {
      PFATAL("Can't open %s", dir_name);
    }
    ck_free(dir_name);
    ...
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("target_state_id:%d, try criu_dump: %d\n", target_state_id, child_pid);
    #endif
    return_code = criu_dump();
    close(img_fd);
  }
  // The "forked-daapd" program cannot use the CRIU DUMP C API, only in command line mode
  else {
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("--------------no API-----------------");
    #endif
    char *xx = alloc_printf("netstat -nxp | grep %d |awk '{ print $7 }'", child_pid);
    int *res_code = NULL;
    int res_code_size = 0;
    get_popen_result(xx, &res_code, &res_code_size);
    ck_free(xx);
    // ACTF("res_code_size:%d", res_code_size);
    char temp[512];

    int sp_cout = sprintf(temp,
      "criu dump --tree %d --images-dir %s -v4 -o dump.log --shell-job --tcp-established ",
      child_pid, dir_name);
    int count_i;
    for (count_i = 0; count_i < res_code_size; count_i++) {
      sp_cout += sprintf(temp+sp_cout, "--external unix[%d] ", res_code[count_i]);
    }
    if (sp_cout >= 512) {
      PFATAL("sp_cout:%d", sp_cout);
    }
    temp[sp_cout] = '\0';

    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("res_code_size:%d\n", res_code_size);
    SNAPSHOT_LOG("dump command:%s\n", temp);
    #endif

    FILE *fp;

    if ((fp = popen(temp, "r")) == NULL) {
      PFATAL("error execute criu dump");
    } else {
      pclose(fp);
    }
  ...
  if (return_code < 0) {
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("Dump Failed Fault\n");
    #endif
    what_err_ret_mean(return_code);
    clear_child();
    if(net_protocol==PRO_TCP)
      close(socket_fd);
    ...
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("read(fsrv_st_fd, &status, 4) OK\n");
    #endif
    ...
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG_LEAVE("self_criu_dump(Dump Failed Fault)");
    #endif
    return DUMP_FAILED_ERR;
  }
  ...
  child_pid_snapshot = child_pid;
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("record child pid after dump: child_pid_snapshot = %d;\n",child_pid);
  #endif

  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG_LEAVE("self_criu_dump()");
  #endif
  return OK;
}
...
static u8 run_target(char** argv, u32 timeout) {
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG_ENTER("run_target()");
  #endif
  run_target_count++;
  run_target_count_current++;
  #ifdef SNAPSHOT_DEBUG
  get_popen_result_no_return("cat /proc/meminfo | head -n 3 ");
  #endif
  #ifdef SNAPSHOT_DEBUG
  if(check_target_process()!=1) PFATAL("error target process num!");
  if(check_zombies()) PFATAL("zombies ate your brain\n");
  #endif

  static struct itimerval it;
  static struct itimerval it_rst;
  static u32 prev_timed_out = 0;
  static u64 exec_ms = 0;

  int status = 0;
  sres __res; // result of all substep
  u32 tb4;
  ...
  if (dumb_mode == 1 || no_forkserver) {
    ...
    //if have snapshot, criu restore, restore child_pid
    if (use_snapshot && can_snapshot_flag && has_snapshot_flag) {
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("use_snapshot && can_snapshot_flag && has_snapshot_flag, just reset child_pid\n");
      #endif
      child_pid = child_pid_snapshot;
    }
    else {
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("not use_snaphost or can not snapshot or not have snapshot, try fork\n");
      #endif
      ...
      if ((res = write(fsrv_ctl_fd, &prev_timed_out, 4)) != 4) {
        ...
        if (stop_soon) {
          #ifdef SNAPSHOT_DEBUG
          SNAPSHOT_LOG_LEAVE("run_target(Write to forkserver Fault)");
          #endif
          return 0;
        } 
      ...
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("tell forkserver to fork OK\n");
      ...
      if ((res = read(fsrv_st_fd, &child_pid, 4)) != 4) {
        ...
        if (stop_soon) {
          #ifdef SNAPSHOT_DEBUG
          SNAPSHOT_LOG_LEAVE("run_target(Read from forkserver Fault)");
          #endif
          return 0;
        }
        RPFATAL(res, "Unable to request new process from fork server (OOM?)");
      ...
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("forkserver return child pid %d\n",child_pid);
      check_target_process();
      #endif
      
      if(kill(child_pid,0)){
        #ifdef SNAPSHOT_DEBUG
        SNAPSHOT_LOG("child pid >0 but target process not exist");
        check_process();
        #endif
        PFATAL("run_target(Fake Target Process Fault)");
      }
      
    }
  }

  //determine if take snapshot 
  if (use_snapshot && can_snapshot_flag) {
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("can snapshot\n");
    #endif
    
    if (has_snapshot_flag == 1) {// if has snapshot
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("and have snapshot, so I'll restore\n");
      #endif

      timer_start(&it_rst,restore_tms);
      clock_start();
      __res = self_criu_restore();
      clock_close_depends(2,__res);
      timer_close(&it_rst);
      ...
      if (__res>0) {
       backstep = 2;
        // stop_soon = 7;
        #ifdef SNAPSHOT_DEBUG
        SNAPSHOT_LOG("erro no: %d\n",__res);
        SNAPSHOT_LOG_LEAVE("run_target(Restore Fault)");
        #endif
        return 0;
      }
    } else if (has_snapshot_flag == 0) { // no snapshot
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("but not have snapshot, so I'll send_over_network_m1()\n");
      #endif

      // Clean up the bitmap of m1
      memset(trace_bits_m1, 0, MAP_SIZE);

      // Send m1 message sequence
      timer_start(&it,timeout);
      clock_start();
      __res = send_over_network_m1();
      clock_close_depends(0,__res);
      timer_close(&it);
      ...
      if (__res>0) {
       backstep = 2;
        ...
        #ifdef SNAPSHOT_DEBUG
        SNAPSHOT_LOG("erro no: %d\n",__res);
        SNAPSHOT_LOG_LEAVE("run_target(Send M1 Fault)");
        #endif
        return 0;
      }
      #ifdef SNAPSHOT_DEBUG
      ...
      // Backup the bitmap generated by m1
      memcpy(trace_bits_m1, trace_bits, MAP_SIZE);

      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("send_over_network_m1() OK, I'll self_criu_dump()\n");
      #endif

      clock_start();
      __res = self_criu_dump();
      clock_close_depends(3,__res);
      ...
      if (__res>0) {
       backstep = 2;
        ...
        #ifdef SNAPSHOT_DEBUG
        SNAPSHOT_LOG("erro no: %d\n",__res);
        SNAPSHOT_LOG_LEAVE("run_target(Dump Fault)");
        #endif
        return 0;
      }
      ...
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("dump ok, so I'll self_criu_restore()\n");
      #endif
      ...
      PFATAL("error has_snapshot_flag value");
    }
    ...
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("restore ok, now I'll send_over_network_m23()\n");
    #endif

    // restore bitmap before sending m23
    memcpy(trace_bits, trace_bits_m1, MAP_SIZE);
    
    timer_start(&it,timeout);
    clock_start();
    status = send_over_network_m23();
    clock_close(1);

    // determine if continue to take snapshot ljq
    if ((time_record[2] > (time_record[0] + minchange)) && (is_first_run_of_fuzz_one) ) {
      can_snapshot_flag = 0;
      has_snapshot_flag = 0;
      delete_kl_messages(kl_messages_m1);
      delete_kl_messages(kl_messages_m2_m3);
    }
    is_first_run_of_fuzz_one = 0;
  }else { // cannot snapshot
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("cannot snapshot, so I'll use AFLNET\n");
    #endif
    ...
    if (dumb_mode == 1 || no_forkserver) {
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("dump_mode==1 and no_forkserver\n");
      #endif
      ...
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("dump_mode!=1 and use_forkserver\n");
      #endif

      if (use_net) {
        send_over_network();
        #ifdef SNAPSHOT_DEBUG
        SNAPSHOT_LOG("send_over_network() OK\n");
        #endif
      }
      ...
      if ((res = read(fsrv_st_fd, &status, 4)) != 4) {
        #ifdef SNAPSHOT_DEBUG
        SNAPSHOT_LOG("read(fsrv_st_fd, &status, 4)!=4\n");
        #endif

        if (stop_soon) {
          #ifdef SNAPSHOT_DEBUG
          SNAPSHOT_LOG("stop_soon>0\n");
          #endif
          #ifdef SNAPSHOT_DEBUG
          SNAPSHOT_LOG_LEAVE("run_target()");
          #endif
          return 0;
        }
        RPFATAL(res, "Unable to communicate with fork server (OOM?)");
  ...
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("all messages sent, start to analyze status...\n");
  #endif
  ...
  if (WIFSIGNALED(status) && !stop_soon) {
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("WIFSIGNALED(status) && !stop_soon\n");
    #endif
    kill_signal = WTERMSIG(status);

    if (child_timed_out && kill_signal == SIGKILL) {
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("child_timed_out && kill_signal == SIGKILL\n");
      #endif
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG_LEAVE("run_target()");
      #endif
    ...
    if (kill_signal == SIGTERM) {
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("kill_signal == SIGTERM\n");
      #endif
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG_LEAVE("run_target()");
      #endif
  ...
  if (uses_asan && WEXITSTATUS(status) == MSAN_ERROR) {
    kill_signal = 0;
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("uses_asan && WEXITSTATUS(status) == MSAN_ERROR\n");
    #endif
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG_LEAVE("run_target()");
    #endif
  ...
  if ((dumb_mode == 1 || no_forkserver) && tb4 == EXEC_FAIL_SIG) {
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("(dumb_mode == 1 || no_forkserver) && tb4 == EXEC_FAIL_SIG\n");
    #endif
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG_LEAVE("run_target()");
    #endif
...
static u8 calibrate_case(char** argv, struct queue_entry* q, u8* use_mem,
  ...
  #ifdef SNAPSHOT_DEBUG
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG_ENTER("calibrate_case");
  #endif
  #endif
...
abort_calibration:
  ...
  #ifdef SNAPSHOT_DEBUG
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG_LEAVE("calibrate_case");
  #endif
  #endif
  return fault;
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG_ENTER("save_if_interesting");
  #endif
  u8 *fn = "";
  u8  hnb;
  //s32 fd;
  ...
  if (fault == crash_mode) {
    ...
    if (!(hnb = has_new_bits(virgin_bits))) {
      ...
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG_LEAVE("save_if_interesting");
      #endif
      return 0;
    }
...
static void write_stats_file(double bitmap_cvg, double stability, double eps) {
  ...
  fprintf(f, "start_time        : %llu\n"
    ...
    orig_cmdline, slowest_exec_ms, messages_sent_total);
  /* ignore errors */
...
static void show_stats(void) {
  ...
  double avg_message_sent =(double)(messages_sent_total-messages_sent_total_last)* 1000 / (cur_ms - last_ms);
...
#define SP20    SP10 SP10
  ...
  SAYF(bV bSTOP "   bit flips : " cRST "%-38s" bSTG bV bSTOP "    levels : "
    cRST "%-10s " bSTG bV "\n", tmp, DI(max_depth));
  ...
  if(can_snapshot_flag && has_snapshot_flag==1)
    ...
    sprintf(tmp, "%d",messages_sent);
  SAYF(bSTG bV bSTOP " message chain len : " cRST "%-56s " bSTG bV bSTOP"\n",tmp);
  sprintf(tmp, "%s/%s (%0.02f%%)", DI(total_execs_current), DI(run_target_count_current), ((double)total_execs_current) * 100 / run_target_count_current);
  SAYF(bSTG bV bSTOP "   running status* : " cRST "%-56s " bSTG bV bSTOP"\n", tmp);
  sprintf(tmp, "%s/%s (%0.02f%%)", DI(restore_ok_count_current), DI(try_restore_count_current), ((double)restore_ok_count_current) * 100 / (try_restore_count_current>0?try_restore_count_current:1));
  SAYF(bSTG bV bSTOP "   restore status* : " cRST "%-56s " bSTG bV bSTOP"\n", tmp);
  sprintf(tmp, "%s/%s (%0.02f%%)", DI(total_execs), DI(run_target_count), ((double)total_execs) * 100 / run_target_count);
  SAYF(bSTG bV bSTOP "    running status : " cRST "%-56s " bSTG bV bSTOP"\n", tmp);
  sprintf(tmp, "%s/%s (%0.02f%%)", DI(restore_ok_count), DI(try_restore_count), ((double)restore_ok_count) * 100 / (try_restore_count>0?try_restore_count:1));
  SAYF(bSTG bV bSTOP "    restore status : " cRST "%-56s " bSTG bV bSTOP"\n", tmp);
  SAYF(bSTG bV bSTOP "    total msg sent : " cRST "%-56s " bSTG bV bSTOP"\n", DI(messages_sent_total));
  messages_sent_total_last = messages_sent_total;
  sprintf(tmp, "%-.2f msg/sec", avg_message_sent);
  SAYF(bSTG bV bSTOP " msg sending speed : " "%s" "%-56s " bSTG bV bSTOP"\n", avg_message_sent>avg_exec*10?cLGN:cRST, tmp);
  SAYF(bSTG bLB bH20 bH20 bH20 bH10 bH5 bH2 bH bRB bSTOP cGRA "\n");  
  ...
  if (cpu_core_count) {
    ...
    if (cpu_aff >= 0) {
      ...
      SAYF("   " cGRA "[cpu%03u:%s%3u%%" cGRA "]\r" cRST,
        MIN(cpu_aff, 999), cpu_color,
        MIN(cur_utilization, 999));

    } else {

      SAYF("   " cGRA "   [cpu:%s%3u%%" cGRA "]\r" cRST,
...
EXP_ST u8 common_fuzz_stuff(char** argv, u8* out_buf, u32 len) {
  ...
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG_ENTER("common_fuzz_stuff()");
  #endif
  u8 fault;
  ...
  if (post_handler) {
    ...
    if (!out_buf || !len) {
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("!out_buf || !len\n");
      #endif
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG_LEAVE("common_fuzz_stuff()");
      #endif
      return 0;
    } 
  ...
  //construct messages m1 m23
  if (use_snapshot && can_snapshot_flag) {

    delete_kl_messages(kl_messages_m1);
    delete_kl_messages(kl_messages_m2_m3);
    u8 m1_flag = 1;
    kl_messages_m1 = construct_kl_messages_mx(kl_messages, M2_prev, m1_flag);

    m1_flag = 2;
    kl_messages_m2_m3 = construct_kl_messages_mx(kl_messages, M2_prev, m1_flag);
  ...
  if (state_aware_mode) {
    update_fuzzs();
    //update value fuzzs of the corresponding region
    if (use_snapshot && (mca_algo == MCA_SMART) && (current_choose_position != -1)) queue_cur->regions_scores[current_choose_position].fuzzs++;
  }

  if (stop_soon){
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("stop_sonn\n");
    #endif
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG_LEAVE("common_fuzz_stuff()");
    #endif
  ...
  if (fault == FAULT_TMOUT) {
    ...
    if (subseq_tmouts++ > TMOUT_LIMIT) {
      cur_skipped_paths++;
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("subseq_tmouts++ > TMOUT_LIMIT\n");
      #endif
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG_LEAVE("common_fuzz_stuff()");
      #endif
  ...
  if(use_snapshot && backstep==1){
    // backstep=1，Means a common failure, try to restore again, more than three times is considered a more serious failure
    // The case of backstep=1: restore fails when snapshot_flag=1
    backstep_count++;
    if(backstep_count>3){
      backstep_count = 0;
      backstep = 2;
  ...
  if(use_snapshot && backstep==2){
    // backstep=2，Means a more serious failure, try to take a snapshot again, and fall back to AFLNET if it exceeds three times
    // The case of backstep=2: m1 failure、dump failure、restore failure when snapshot_flag=0
    has_snapshot_flag = 0;
    serious_err_count++;
    if(serious_err_count>3){
      can_snapshot_flag = 0;
      // delete_kl_messages(kl_messages_m1);
      // delete_kl_messages(kl_messages_m2_m3);
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("backstep=2 too many times, try aflnet\n");
      #endif
    }else{
      can_snapshot_flag = 1;
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("backstep=2, next, re-snapshot\n");
      #endif
    }
  }
  
  if(use_snapshot && backstep==0){
    backstep_count = (backstep_count==0)?0:(backstep_count-1);
  }
  ...
  if(use_snapshot &&  stop_soon==7) {
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG_LEAVE("common_fuzz_stuff(return 1)");
    #endif
    return 1;
  }
    
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG_LEAVE("common_fuzz_stuff(return 0)");
  #endif
  return 0;
...
static u8 fuzz_one(char** argv) {
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG_ENTER("fuzz_one()");
  #endif
  run_target_count_current =0 ;
  total_execs_current = 0;
  try_restore_count_current = 0;
  restore_ok_count_current = 0;
  backstep_count = 0;
  serious_err_count = 0;
  ...
  if (pending_favored) {
    ...
    if ((queue_cur->was_fuzzed || !queue_cur->favored) &&
        ...
        #ifdef SNAPSHOT_DEBUG
        SNAPSHOT_LOG("(queue_cur->was_fuzzed || !queue_cur->favored) && UR(100) < SKIP_TO_NEW_PROB\n");
        #endif
        #ifdef SNAPSHOT_DEBUG
        SNAPSHOT_LOG_LEAVE("fuzz_one()");
        #endif
        return 1;
      } 
    ...
    if (queue_cycle > 1 && !queue_cur->was_fuzzed) {
      ...
      if (UR(100) < SKIP_NFAV_NEW_PROB) {
        #ifdef SNAPSHOT_DEBUG
        SNAPSHOT_LOG("UR(100) < SKIP_NFAV_NEW_PROB\n");
        #endif
        #ifdef SNAPSHOT_DEBUG
        SNAPSHOT_LOG_LEAVE("fuzz_one()");
        #endif
      ...
      if (UR(100) < SKIP_NFAV_OLD_PROB) {
        #ifdef SNAPSHOT_DEBUG
        SNAPSHOT_LOG("UR(100) < SKIP_NFAV_OLD_PROB\n");
        #endif
        #ifdef SNAPSHOT_DEBUG
        SNAPSHOT_LOG_LEAVE("fuzz_one()");
        #endif
...
AFLNET_REGIONS_SELECTION:;
  ...
  if (state_aware_mode) {
    ...
    if (use_snapshot) {
      s32 M2_start_region_ID_PREV[total_region];
      memset(M2_start_region_ID_PREV, -1, total_region*sizeof(s32));

      s32 M2_start_region_ID_PREV_index = 0;
      ...
      if (target_state_id == 0) {
        ...
        for (i = 0; i < queue_cur->region_count; i++) {
          ...
          if (regionalStateCount > 0) {
            ...
            if (mca_algo == MCA_SMART) queue_cur->regions_scores[i].message_depth = i;
            ...
            regionalStateCount_flag = 1;
            if (reachableStateID == target_state_id) {
              M2_start_region_ID_PREV[M2_start_region_ID_PREV_index] = i;
              M2_start_region_ID_PREV_index++;
            }
          }
        }

        if (regionalStateCount_flag == 0) {
          //No annotation for this region
          #ifdef SNAPSHOT_DEBUG
          SNAPSHOT_LOG("regionalStateCount_flag == 0\n");
          #endif
          #ifdef SNAPSHOT_DEBUG
          SNAPSHOT_LOG_LEAVE("fuzz_one()");
          #endif
        ...
        if (M2_start_region_ID_PREV[0] != -1) {
          current_choose_position = message_chain_analysis(M2_start_region_ID_PREV, M2_start_region_ID_PREV_index, mca_algo);
          #ifdef SNAPSHOT_DEBUG
          SNAPSHOT_LOG("current_choose_position:%d\n", current_choose_position);
          #endif
          M2_start_region_ID = current_choose_position + 1;
          if (mca_algo == MCA_SMART) queue_cur->regions_scores[current_choose_position].selected_times++;
        }
        //Handle corner case(s) and skip the current queue entry
        else {
          #ifdef SNAPSHOT_DEBUG
          SNAPSHOT_LOG("M2_start_region_ID_PREV[0] == -1\n");
          #endif
          #ifdef SNAPSHOT_DEBUG
          SNAPSHOT_LOG_LEAVE("fuzz_one()");
          #endif
        ...
        if (M2_start_region_ID >= queue_cur->region_count) {
          #ifdef SNAPSHOT_DEBUG
          SNAPSHOT_LOG("M2_start_region_ID >= queue_cur->region_count\n");
          #endif
          #ifdef SNAPSHOT_DEBUG
          SNAPSHOT_LOG_LEAVE("fuzz_one()");
          #endif
      ...
      if (target_state_id == 0) {
        ...
        for(i = 0; i < queue_cur->region_count; i++) {
          ...
          if (regionalStateCount > 0) {
            ...
            //No annotation for this region
            #ifdef SNAPSHOT_DEBUG
            SNAPSHOT_LOG("regionalStateCount <= 0\n");
            #endif
            #ifdef SNAPSHOT_DEBUG
            SNAPSHOT_LOG_LEAVE("fuzz_one()");
            #endif
  ...
  send_over_network_m1_length = M2_start_region_ID;
  // kliter_t(lms) *it_temp;
  if (use_snapshot) {
    is_first_run_of_fuzz_one = 1;
    if (M2_prev == NULL) {
      has_snapshot_flag = 0;
      can_snapshot_flag = 0;
    } else {
...
havoc_stage:
  ...
  if (use_snapshot && can_snapshot_flag) MAX_STAGE_CUR = stage_max_factor * stage_max;
  else MAX_STAGE_CUR = stage_max;

  for (stage_cur = 0; stage_cur < MAX_STAGE_CUR; stage_cur++) {
    ...
    if (use_snapshot) {
      if (MAX_BUF_SIZE != 0 && temp_len > MAX_BUF_SIZE) {
        temp_len = MAX_BUF_SIZE;
        u8 *out_buf_max = ck_alloc_nozero(temp_len);
        memcpy(out_buf_max, out_buf, temp_len);
        ck_free(out_buf);
        out_buf = ck_alloc_nozero(temp_len);
        memcpy(out_buf, out_buf_max, temp_len);
        ck_free(out_buf_max);
...
abandon_entry:
  ...
  if (state_aware_mode && use_snapshot && (mca_algo == MCA_SMART)) {
    current_choose_position = -1;
  }
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG("abandon_entry\n");
  #endif
...
static void handle_timeout(int sig) {
  ...
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG_ENTER("handle_timeout()");
  #endif
  if (is_restoring) {
    #ifdef SNAPSHOT_DEBUG
    SNAPSHOT_LOG("is restoring...So I'll kill criu and clear all iptables and reset timer\n");
    #endif
    is_restoring = 2;
    system("kill -9 `pidof criu`");
  }
  else{
    if (child_pid > 0) {

      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("child_pid>0...So I'll kill -9 child_pid and set child_timed_out =1\n");
      #endif
      child_timed_out = 1;
      kill(child_pid, SIGKILL);
      ...
      #ifdef SNAPSHOT_DEBUG
      SNAPSHOT_LOG("child_pid == -1 && forksrv_pid > 0...So I'll kill -9 froksrv_pid and set child_timed_out =1\n");
      #endif
      child_timed_out = 1;
  ...
  #ifdef SNAPSHOT_DEBUG
  SNAPSHOT_LOG_LEAVE("handle_timeout()");
  #endif
}
...
static void usage(u8* argv0) {
  ...
  SAYF("\n%s [ options ] -- /path/to/fuzzed_app [ ... ]\n\n"
    ...
    "SNPSFuzzer:\n\n"
    "  -b restore_tms - enable snapshot mode, resotore_tms: timeout for restore(ms)\n"
    "  -a mca_algo    - mca selection algorithm (See aflnet.h for all available options)\n\n"
...
static void setup_snapshot_dir() {

  DIR *dirptr = NULL;
  // u8* dir_name = alloc_printf("state%d_images",target_state_id);
  dirptr = opendir("snapshot_dir");
  if(dirptr == NULL){
    if(mkdir("snapshot_dir",0755) == -1) PFATAL("mkdir snapshot_dir error");
  }

  closedir(dirptr);

  //write to log file
  u8* fn = alloc_printf("snapshot_dir/restore_command_log.txt");

  snapshot_logfile_fd = open(fn, O_WRONLY | O_CREAT | O_TRUNC, 0444);
  ck_free(fn);

  if (snapshot_logfile_fd < 0) FATAL("open snapshot_dir/restore_command_log.txt error");

  snapshot_logfile = fdopen(snapshot_logfile_fd, "w");
  setvbuf(snapshot_logfile,NULL,_IOFBF,4096);

  if (!snapshot_logfile) {
    close(snapshot_logfile_fd);
    FATAL("fdopen snapshot_dir/restore_command_log.txt error");
  }

  u8 *fn1 = alloc_printf("snapshot_dir/prefix_save.txt");
  prefix_save_logfile_fd = open(fn1, O_WRONLY | O_CREAT | O_TRUNC, 0444);
  ck_free(fn1);

  if (prefix_save_logfile_fd < 0) FATAL("open snapshot_dir/prefix_save.txt error");

  prefix_save_logfile = fdopen(prefix_save_logfile_fd, "w");

  if (!prefix_save_logfile) {
    close(prefix_save_logfile_fd);
    FATAL("fdopen snapshot_dir/snapshot_dir/prefix_save.txt error");
  }
...
void snapshot_log(unsigned int loglevel, const char *format, ...) {
  va_list marker;
  va_start(marker,format);
  vfprintf(snapshot_logfile,format,marker);
...
int main(int argc, char** argv) {
  gettimeofday(&log_time,NULL);
  log_time_start = log_time.tv_sec*1000+log_time.tv_usec/1000;
  #ifdef SNAPSHOT_DEBUG
  ACTF("Running on SNAPSHOT_DEBUG mode!");
  #endif
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:QN:D:W:w:P:KEq:s:RFc:l:b:a:")) > 0)
    ...
    case 'b': /* use_snapshot */
      if (use_snapshot) FATAL("Multiple -b options not supported");
      restore_tms = atoi(optarg);
      use_snapshot = 1;
      break;

    case 'a': /* message chain analysis selection option */
      if (sscanf(optarg, "%hhu", &mca_algo) < 1 || optarg[0] == '-') FATAL("Bad syntax used for -a");
      break;
  ...
  if (use_snapshot) {
    if (!use_net) FATAL("-b must be used with -N");
  }
  ...
  if (use_snapshot) {
    ...
    iptables_del = alloc_printf("iptables -t filter -D OUTPUT -p tcp --sport %u -j DROP; iptables -t filter -D INPUT -p tcp --sport %u -j DROP; iptables -t filter -D OUTPUT -p tcp --dport %u -j DROP; iptables -t filter -D INPUT -p tcp --dport %u -j DROP", net_port, net_port, net_port, net_port);
    #ifdef SNAPSHOT_DEBUG
    libsoccr_set_log(SOCCR_LOG_DBG,snapshot_log);
    #endif
  }
  ...
  if (state_aware_mode) {
    ...
    while (1) {
      ...
      while(!selected_seed || selected_seed->region_count == 0) {
        ...
        if (use_snapshot) {
          // has_snapshot_flag = check_state_snapshot();
          has_snapshot_flag = 0;
          can_snapshot_flag = 0;
        }
...
stop_fuzzing:
  ...
  if (use_snapshot) {
    ...
    fclose(prefix_save_logfile);
  }
  destroy_queue();
  destroy_extras();
  ck_free(target_path);
  ck_free(sync_id);
  ck_free(iptables_add);
  ck_free(iptables_del);
